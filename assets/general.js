...
*/
   ...
   console.log(
  "%cCUSTOM SHOPIFY THEME DEVELOPED BY ECOMGRADUATES LLC",
  "-webkit-text-stroke: 0.2vw #a61b30;    font-family: montserrat;    font-size: 2vw;    padding: 3px 0 13px 31px;    letter-spacing: 0vw;    text-transform: uppercase;    background: rgb(226,69,212);    background: linear-gradient(90deg, rgba(226,69,212,1) 3%, rgba(223,28,127,1) 48%, rgba(213,36,42,1) 98%);    margin: 0;    text-align: left;    font-weight: 700;    color: #fff;    text-shadow: -3px 4px 13px #460f17, -11px 7px 20px #080203;"
);
...
/*
    Second image on hover effect (product list items)
*/
...
*/
document.querySelectorAll('.image-columns .img-wrapper').forEach(wrapper => {
    console.log('wrapper')
    wrapper.addEventListener('mouseover', () => {
        wrapper.querySelector('.img-hover').style.opacity = '1'
    })
    wrapper.addEventListener('mouseleave', () => {
        wrapper.querySelector('.img-hover').style.opacity = '0'
    })
})